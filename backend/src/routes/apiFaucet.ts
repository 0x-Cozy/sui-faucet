import { Router, Request, Response } from 'express'
import { apiKeyAuth } from '../middleware/apiKeyAuth'
import { logApiUsage } from '../services/apiKeyService'
import { checkRateLimit, consumeRateLimit } from '../services/rateLimiter'
import { sendTokens, validateWalletAddress, getUserBalance } from '../services/suiService'
import { logRequest } from '../services/historyService'

interface ApiRequest extends Request {
  apiApp?: any
}

const router = Router()

// api faucet request (no captcha)
router.post('/v1/faucet/request', apiKeyAuth, async (req: ApiRequest, res: Response) => {
  const ip = req.ip || 
             req.headers['x-forwarded-for'] || 
             req.headers['x-real-ip'] || 
             req.connection.remoteAddress || 
             req.socket.remoteAddress || 
             'unknown'
  
  const realIP = Array.isArray(ip) ? ip[0] : ip

  try {
    const { walletAddress, amount } = req.body
    const app = req.apiApp
    
    if (!walletAddress || !amount) {
      await logApiUsage(app.id, '/v1/faucet/request', realIP, false, 'missing parameters')
      return res.status(400).json({
        success: false,
        error: 'wallet address and amount required'
      })
    }
    
    // check rate limits (same as frontend)
    const rateLimitInfo = await checkRateLimit(realIP, walletAddress)
    
    if (rateLimitInfo.blocked) {
      await logApiUsage(app.id, '/v1/faucet/request', realIP, false, 'rate limit exceeded')
      return res.status(429).json({
        success: false,
        error: 'rate limit exceeded'
      })
    }
    
    // consume rate limit
    await consumeRateLimit(realIP, walletAddress)
    
    // send tokens (same as frontend)
    const txHash = await sendTokens(walletAddress, amount)
    
    // log successful request
    await logRequest({
      walletAddress,
      amount,
      source: 'api',
      ip: realIP,
      success: true,
      txHash
    })
    
    await logApiUsage(app.id, '/v1/faucet/request', realIP, true)
    
    res.json({
      success: true,
      data: {
        txHash,
        message: `${amount} SUI sent successfully`
      }
    })
  } catch (error: any) {
    console.error('api faucet request error:', error)
    const app = req.apiApp
    await logApiUsage(app.id, '/v1/faucet/request', realIP, false, error.message)
    
    res.status(500).json({
      success: false,
      error: error.message || 'failed to process faucet request'
    })
  }
})

// api balance check
router.get('/v1/faucet/balance/:address', apiKeyAuth, async (req: ApiRequest, res: Response) => {
  const ip = req.ip || 
             req.headers['x-forwarded-for'] || 
             req.headers['x-real-ip'] || 
             req.connection.remoteAddress || 
             req.socket.remoteAddress || 
             'unknown'
  
  const realIP = Array.isArray(ip) ? ip[0] : ip

  try {
    const { address } = req.params
    const app = req.apiApp
    
    const balance = await getUserBalance(address)
    
    await logApiUsage(app.id, '/v1/faucet/balance', realIP, true)
    
    res.json({
      success: true,
      data: { balance }
    })
  } catch (error: any) {
    console.error('api balance check error:', error)
    const app = req.apiApp
    await logApiUsage(app.id, '/v1/faucet/balance', realIP, false, error.message)
    
    res.status(500).json({
      success: false,
      error: error.message || 'failed to check balance'
    })
  }
})

export default router 